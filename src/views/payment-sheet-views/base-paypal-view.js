...
BasePayPalView.prototype.initialize = function () {
  ...
  return btPaypal.create({client: this.client}).then(function (paypalInstance) {
    ...
    Object.keys(window.paypal.FUNDING).forEach(function (key) {
      if (key === 'PAYPAL' || key === 'CREDIT') {
        return;
      }
      checkoutJSConfiguration.funding.disallowed.push(window.paypal.FUNDING[key]);
    });
    ...
    if (isCredit) {
      ...
      checkoutJSConfiguration.funding.disallowed.push(window.paypal.FUNDING.CREDIT);
    }

    buttonSelector = dropinWrapperId + ' ' + buttonSelector;

    return window.paypal.Button.render(checkoutJSConfiguration, buttonSelector).then(function () {
