...
describe('ApplePayView', () => {
  ...
  beforeEach(() => {
    ...
    return testContext.model.initialize().then(() => {
      ...
      window.ApplePaySession = jest.fn().mockReturnValue(testContext.fakeApplePaySession);
      window.ApplePaySession.canMakePayments = jest.fn().mockReturnValue(true);
      window.ApplePaySession.supportsVersion = jest.fn().mockReturnValue(true);
  ...
  describe('initialize', () => {
    ...
    describe('button click handler', () => {
      ...
      test('creates an ApplePaySession with the payment request', () => {
        ...
        expect(window.ApplePaySession).toBeCalledWith(2, testContext.fakePaymentRequest);
      });
      ...
      test('can set which version of ApplePaySession to use', () => {
        ...
        expect(testContext.view.applePayInstance.createPaymentRequest).toBeCalledWith(testContext.fakePaymentRequest);
        expect(window.ApplePaySession).toBeCalledWith(3, testContext.fakePaymentRequest);
      });
          ...
          test(
            'completes payment on ApplePaySession with status success',
              ...
              testContext.fakeApplePaySession.completePayment = status => {
                expect(status).toBe(window.ApplePaySession.STATUS_SUCCESS);
          ...
          test(
            'completes payment on ApplePaySession with status failure and reports the error',
              ...
              testContext.fakeApplePaySession.completePayment = status => {
                ...
                expect(testContext.view.model.reportError).toBeCalledWith(fakeError);
                expect(status).toBe(window.ApplePaySession.STATUS_FAILURE);
                done();
              };
  ...
  describe('isEnabled', () => {
    ...
    test('resolves with false when device cannot make payments', () => {
      window.ApplePaySession.canMakePayments.mockReturnValue(false);
    ...
    test(
      ...
      () => {
        window.ApplePaySession.supportsVersion.mockReturnValue(false);
    ...
    test('defaults ApplePaySession version to 2', () => {
      return ApplePayView.isEnabled(testContext.options).then(() => {
        expect(window.ApplePaySession.supportsVersion).toBeCalledTimes(1);
        expect(window.ApplePaySession.supportsVersion).toBeCalledWith(2);
      });
    });
    ...
    test('can set ApplePaySession version', () => {
      ...
      return ApplePayView.isEnabled(testContext.options).then(() => {
        expect(window.ApplePaySession.supportsVersion).toBeCalledTimes(1);
        expect(window.ApplePaySession.supportsVersion).toBeCalledWith(3);
      });
    });
