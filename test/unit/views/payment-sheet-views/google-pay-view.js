...
describe('GooglePayView', () => {
  ...
  describe('initialize', () => {
    ...
    test(
      ...
      () => {
        ...
        jest.spyOn(window.google.payments.api, 'PaymentsClient');

        return testContext.view.initialize().then(() => {
          expect(window.google.payments.api.PaymentsClient).toBeCalledWith({
  ...
  describe('isEnabled', () => {
    ...
    test(
      'loads Google Payment script file when google window does not exist',
      () => {
        const storedFakeGoogle = window.google;

        delete window.google;

        assets.loadScript.mockImplementation(() => {
          window.google = storedFakeGoogle;
    ...
    test(
      'loads Google Payment script file when the payment client on google window does not exist',
      () => {
        const storedFakeGoogle = window.google;
    ...
    test(
      'does not load Google Payment script file if window google pay object already exists',
      () => {
        return GooglePayView.isEnabled(testContext.fakeOptions).then(() => {
