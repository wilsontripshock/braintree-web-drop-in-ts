...
describe('BasePayPalView', () => {
  ...
  beforeEach(() => {
    ...
    window.paypal = testContext.paypal;
  ...
  describe('isEnabled', () => {
    ...
    test('resolves true if window.paypal exists', () => {
      return BasePayPalView.isEnabled(testContext.options).then(result => {
        expect(result).toBe(true);
      });
    });

    test('skips loading paypal script if window.paypal exists', () => {
      return BasePayPalView.isEnabled(testContext.options).then(() => {
        expect(assets.loadScript).not.toBeCalled();
      });
    });

    test('loads paypal script if window.paypal does not exist', () => {
